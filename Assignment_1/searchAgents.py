...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            # check if a wall is being hit
            hitsWall = self.walls[nextx][nexty]
            if hitsWall == False:
                nonvisited = []
                old_nonvisited = state[1]
                # check if new position is a non-visited corner
                for elem in old_nonvisited:
                    if elem == (nextx, nexty):
                        continue
                    nonvisited.append(elem)
                # adding new coordinates and the non-visited list to successors
...
def cornersHeuristic(state, problem):
    ...
    # "nonvisited" contains the coordinates of non-visited food locations
    nonvisited = state[1]
    # "currentpos" is the current position
    currentpos = state[0]

    # "distances" stores the Manhattan distance between pacman's position and food
    distances = []
    ...
    # current position is changed to the position of nearest food
